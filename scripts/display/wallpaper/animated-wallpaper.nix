{ pkgs }: 
  pkgs.writeShellApplication { 
    ...
    text = '' 
      ...
      IFS=$'\n' read -r -d ''' -a monitor_name_arr <<< "$monitor_name"
      IFS=$'\n' read -r -d ''' -a monitor_desc_arr <<< "$monitor_description"
	  ...
	  mpvpaper -o "no-audio --loop" "''${monitor_name_arr[i]}" "''${wallpaper_path}''${vertical_wallpaper}" &
	  sleep 0.5
	elif xrandr | grep -e "^''${monitor_name_arr[i]} " | grep -e "right (" -e "left ("; then
	  mpvpaper -o "no-audio --loop" "''${monitor_desc_arr[i]}" "''${wallpaper_path}''${veritcal_wallpaper}" &
	else
